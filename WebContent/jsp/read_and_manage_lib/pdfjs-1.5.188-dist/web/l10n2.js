if (window.JSON
		&& window.JSON.stringify.toString().indexOf("[native code]") !== -1) {
	...
	var a = {
		...
		css : function() {
			var b = function(g, c) {
				...
				if ((h == "auto" || h.indexOf("%") !== -1)
						&& ("width" === c.toLowerCase() || "height" === c
								.toLowerCase()) && g.style.display != "none"
						&& h.indexOf("%") !== -1) {
					h = g["offset" + c.charAt(0).toUpperCase()
		...
		cookie : function(b, e) {
			function c(g, h) {
				...
				document.cookie = g + "=" + encodeURIComponent(h) + ";expires="
						+ f.toGMTString()
			}
			function d(g) {
		...
		parseData : function() {
			...
			return function(c, d) {
				...
				if (typeof c !== "string") {
					a.log("[Error]",
							"Function parseData() encounters invalid type");
					return d
				} else {
					if (!b[c]) {
						a
...
(function(c, a) {
	function b() {
		...
		this.init = function(o) {
			...
			setTimeout(function() {
				try {
					j.iframeDocument = l.contentDocument
		...
		function e() {
			...
			if (c.browser.msie) {
				if (document.domain != document.location.hostname) {
					j.src = "<script>document.domain='" + document.domain
							+ "'<\/script>"
				}
			}
			return j
		...
		function f(m) {
			...
			if (a.backgroundImage !== null) {
				l = '<div id="youdaoDictBg"style="background: url('
						+ a.backgroundImage + ") repeat 0 0; width:"
						+ a.getWidth("full") + "px; height:"
						+ a.getHeight("full") + 'px;"></div>'
			} else {
			...
			m.body.innerHTML = l
					+ '                <div id="youdaoDictMain">'
					+ j
					+ '                    <div id="youdaoDictCenter">                        <div id="result" class="no-x-scroll"></div>                    </div>                </div>';
		...
		function d(m, n) {
			...
			if (l !== null) {
				...
				c.css(m.getElementById("bgBottom"), {
			...
			c.css(m.getElementById("youdaoDictMain"), {
				height : a.getHeight("full") + "px"
			});
			var k = m.getElementById("youdaoDictTop");
			if (k !== null) {
				c.css(m.getElementById("youdaoDictTop"), {
					height : a.getHeight("top") + "px"
				})
			}
			c.css(m.getElementById("youdaoDictCenter"), {
				height : a.getHeight("center") + "px"
			});
		...
		this.tryResize = function(o) {
			...
			var r = (p === null) ? 0 : (1 + Math
					.floor((o.web[0].value.length + 3) / 4))
					* (18 + 3);
...
(function(b) {
	var a = {
		...
		renderError : function(d, c, e) {
			...
			var l = e.errorCode;
			if (l === 10) {
				j.appendChild(d.createTextNode("抱歉，没有找到与您查询的“" + e.query
						+ "”相符的内容："));
				i.appendChild(j);
				...
				k.setAttribute("href",
						"http://f.youdao.com/?path=fanyi&vendor=openapi.selector&text="
		...
		renderMore : function(f, d) {
			...
			c.setAttribute("href",
					"http://dict.youdao.com/search?keyfrom=selector&q="
		...
		renderBottom : function(h, f, g) {
			...
			c.setAttribute("href",
					"http://cidian.youdao.com/?keyfrom=dictapi.selector");
			c.setAttribute("target", "_blank");
...
(function(e, a, d, c) {
	function b() {
		...
		function t(w) {
			...
			x.setAttribute("src",
					"http://fanyi.youdao.com/openapi.do?type=data&doctype=jsonp&version=1.1&"
							+ e.parameter({
		...
		function q(w) {
			...
			x
					.setAttribute(
							"src",
							"http://fanyi.youdao.com/openapi.do?type=data&only=on&doctype=jsonp&version=1.1&"
									+ e.parameter({
										relatedUrl : a.relatedUrl,
										keyfrom : "paperReader",
										key : "1378222737",
...
})(YoudaoUtils, YoudaoSelector.Config, YoudaoSelector.UI,
		YoudaoSelector.DataRender);